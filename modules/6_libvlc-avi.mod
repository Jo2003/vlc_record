...
LIVE_REC     = <<{[%URL%]};;-I;;dummy;;--ignore-config;;--no-http-reconnect;;--http-caching={[%CACHE%]};;--sout=#transcode{vcodec=DIV3,acodec=mp3,ab=128,vb=1400,venc=ffmpeg{keyint=25,hurry-up,vt=5000}}:duplicate{dst=display,dst=std{access=file,mux=ffmpeg{mux={[%MUX%]}},dst='{[%DST%]}.{[%MUX%]}'}}>>
ARCH_REC     = <<{[%URL%]};;-I;;dummy;;--ignore-config;;--no-http-reconnect;;--http-caching={[%CACHE%]};;--sout=#transcode{vcodec=DIV3,acodec=mp3,ab=128,vb=1400,venc=ffmpeg{keyint=25,hurry-up,vt=5000}}:duplicate{dst=display,dst=std{access=file,mux=ffmpeg{mux={[%MUX%]}},dst='{[%DST%]}.{[%MUX%]}'}}>>
LIVE_SIL_REC = <<{[%URL%]};;-I;;dummy;;--ignore-config;;--no-http-reconnect;;--http-caching={[%CACHE%]};;--sout=#transcode{vcodec=DIV3,acodec=mp3,ab=128,vb=1400,venc=ffmpeg{keyint=25,hurry-up,vt=5000}}:std{access=file,mux=ffmpeg,dst='{[%DST%]}.{[%MUX%]}'}>>
ARCH_SIL_REC = <<{[%URL%]};;-I;;dummy;;--ignore-config;;--no-http-reconnect;;--http-caching={[%CACHE%]};;--sout=#transcode{vcodec=DIV3,acodec=mp3,ab=128,vb=1400,venc=ffmpeg{keyint=25,hurry-up,vt=5000}}:std{access=file,mux=ffmpeg,dst='{[%DST%]}.{[%MUX%]}'}>>
