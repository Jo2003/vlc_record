...
LIVE_REC     = <<{[%URL%]};;-I;;dummy;;--ignore-config;;--no-http-reconnect;;--http-caching={[%CACHE%]};;--sout=#transcode{vcodec=DIV3,vb=1400,acodec=mp3,ab=128,samplerate=44100,channels=2,audio-sync,venc=ffmpeg{keyint=75,hurry-up,vt=1400},aenc=twolame{mode=2}}:duplicate{dst=display,dst=std{access=file,mux=ffmpeg{mux={[%MUX%]}},dst='{[%DST%]}.{[%MUX%]}'}}>>
...
# ARCH_REC     = <<{[%URL%]};;-I;;dummy;;--ignore-config;;--no-http-reconnect;;--http-caching={[%CACHE%]};;--sout=#transcode{vcodec=DIV3,acodec=mp3,ab=128,channels=2,samplerate=44100,vb=1400,venc=ffmpeg{keyint=75,hurry-up,vt=5000},aenc=twolame{mode=2}}:duplicate{dst=display,dst=std{access=file,mux={[%MUX%]},dst='{[%DST%]}.{[%MUX%]}'}}>>
# LIVE_SIL_REC = <<{[%URL%]};;-I;;dummy;;--ignore-config;;--no-http-reconnect;;--http-caching={[%CACHE%]};;--sout=#transcode{vcodec=DIV3,acodec=mp3,ab=128,channels=2,samplerate=44100,vb=1400,venc=ffmpeg{keyint=75,hurry-up,vt=5000},aenc=twolame{mode=2}}:std{access=file,mux={[%MUX%]},dst='{[%DST%]}.{[%MUX%]}'}>>
# ARCH_SIL_REC = <<{[%URL%]};;-I;;dummy;;--ignore-config;;--no-http-reconnect;;--http-caching={[%CACHE%]};;--sout=#transcode{vcodec=DIV3,acodec=mp3,ab=128,channels=2,samplerate=44100,vb=1400,venc=ffmpeg{keyint=75,hurry-up,vt=5000},aenc=twolame{mode=2}}:std{access=file,mux={[%MUX%]},dst='{[%DST%]}.{[%MUX%]}'}>>
