...
ARCH_REC     = <<{[%URL%]};;:no-http-reconnect;;:run-time=36000;;:http-caching={[%CACHE%]};;:sout=#transcode{vcodec=xvid,vb=3750,threads=2,acodec=mp3,ab=192,channels=2,samplerate=48000,audio-sync,venc=ffmpeg{keyint=50,hurry-up,hw,vt=1500},aenc=ffmpeg}:duplicate{dst=display,dst=std{access=file,mux=ffmpeg{mux={[%MUX%]}},dst='{[%DST%]}.{[%MUX%]}'}}>>
...
ARCH_SIL_REC = <<{[%URL%]};;:no-http-reconnect;;:run-time=36000;;:http-caching={[%CACHE%]};;:sout=#transcode{vcodec=xvid,vb=3750,threads=2,acodec=mp3,ab=192,channels=2,samplerate=48000,audio-sync,venc=ffmpeg{keyint=50,hurry-up,hw,vt=1500},aenc=ffmpeg}:std{access=file,mux=ffmpeg{mux={[%MUX%]}},dst='{[%DST%]}.{[%MUX%]}'}>>
