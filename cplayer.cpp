/*********************** Information *************************\
| $HeadURL: https://vlc-record.googlecode.com/svn/branches/rodnoe.tv/cplayer.cpp $
|
| Author: Jo2003
|
...
|
| $Id: cplayer.cpp 1501 2015-03-04 09:05:27Z Olenka.Joerg $
\*************************************************************/
...
int CPlayer::initPlayer(const QString &sOpts)
{
   ...
   if (pVlcInstance)
   {
      ...
      if ((pMediaPlayer = libvlc_media_player_new (pVlcInstance)) != NULL)
      {
         ...
         // set volume as last used from missioncontrol ...
         libvlc_audio_set_volume(pMediaPlayer, missionControl.getVolume());
